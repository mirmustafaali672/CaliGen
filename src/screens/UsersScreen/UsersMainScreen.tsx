  ...
  return (
    <View style={{backgroundColor: MaterialColorTheme.surface, flex: 1}}>
      <ObjectScreenHeader
        ...
        createBuutonClickNavigationRoute={'CreateUserScreen'}
        ...
        {!usersActivity && (
          <View style={{flex: 1, padding: 10}}>
            <View style={{flexDirection: 'row'}}>
              ...
              />
              <RobotoText
                text={users?.totalCount.toString() ?? ''}
                textStyle={{}}
                isBold={true}
                numberOfLines={0}
              />
                ...
                renderItem={({item}) => {
                  return (
                    <EntityDetailCardComponent
                      ...
                      value2={`Surname: ${item.surname}`}
                      value3={`Email: ${item.email}`}
                      value4={`Phone Number: ${item.phoneNumber}`}
                      showUserProfile={true}
                      navigationScreenName="CreateUserScreen"
                    />
                  );
                }}
              />
